...
export default function Home() {
  ...
  const { InputSettings, SetInputSettings, SetViewSettings, viewSettings } =
    useSettings();
  ...
  useEffect(() => {
    ...
    console.log(id);
    if (id) templateProject(id);
  }, [searchparams]);

  return (
    <Suspense fallback={<Loading/>}>
      <div className="bg-zinc-200 text-black flex flex-col h-screen ">
        <header className="bg-zinc-950 text-white p-2">
          <Link href={"/"}>
            <h2 className="font-bold">YOKOCODE</h2>
          </Link>
        </header>
